...
RSpec.describe 'StatusTest' do
  ...
  it "shows per queue retry count" do
    retries = %w(bulk bulk something-else).map { |q| instance_double('Sidekiq::Job', queue: q) }
  ...
  it "shows per queue scheduled count" do
    scheduled = %w(bulk bulk something-else).map { |q| instance_double('Sidekiq::Job', queue: q) }
